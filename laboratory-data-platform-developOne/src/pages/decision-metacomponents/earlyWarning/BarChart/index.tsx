...
import {
  getEarlyWaringOptionOnRedux,
  getEarlyWaringReturnOnRedux,
} from '../service';
...
const BarChart = (props: { firstLevelPathKey: string }): ReactElement => {
  ...
  const firstLevelPathValue =
    firstLevelMenuEnum[firstLevelPathKey as keyof typeof firstLevelMenuEnum];
  ...
  const barChartHeight: number =
    YAxisDataCurrent.length > 20 ? YAxisDataCurrent.length * 20 : 400;
    ...
    switch (firstLevelPathKey) {
      case firstLevelPaths[0]: // 预警页人力链
        companyData?.forEach((item) => {
          ...
          if (
            item.skill &&
            item[attributesResult] &&
            (alarmTypeOption === item['alarmType'] || alarmTypeOption === 2)
          ) {
            yAxisData.push(`${formatDate(item.eventTime)}/${item.skill}`);
            seriesData.push({
              value: item[attributesResult],
              itemStyle: {
                color: item.alarmType === 1 ? REDCOLOR : BLUECOLOR,
              },
            });
          }
        });
      ...
      case firstLevelPaths[4]: // 预警页销售链
        companyData?.forEach((item) => {
          ...
          if (
            ...
            yAxisData.push(
              `${formatDate(item.eventTime)}：${productTypes[item.categories]}`,
            );
            seriesData.push({
              value: item[attributesResult],
              itemStyle: {
   ...
   */
  const paginationOnChange: PaginationProps['onChange'] = (
    page: number,
    pageSize: number,
  ) => {
    setCurrent(page);
    ...
    setYAxisDataCurrent(
      YAxisData.slice(pageSize * (page - 1), pageSize * page),
    );
  };
  ...
  useEffect(() => {
    if (
      earlyWarningReturn !== undefined &&
      earlyWarningOption?.company !== undefined
    ) {
      // 获取柱状图数据并更新
      const { yAxisData, seriesData } = getChartData(
        firstLevelPathKey,
        earlyWarningReturn[earlyWarningOption.company],
      );
  ...
  console.log('barChartOption', barChartOption);
  return (
    <div
      css={{
        marginBottom: '40px',
