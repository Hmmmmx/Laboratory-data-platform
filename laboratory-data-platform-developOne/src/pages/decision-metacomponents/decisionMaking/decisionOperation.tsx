...
import { update, getEarlyWaring, getEarlyWaringOptionOnRedux, getEarlyWaringReturnOnRedux } from '../earlyWarning/service';
import type { ReactElement } from "react";
import dayjs from 'dayjs';

const Operation = (props: { firstLevelPathKey: string, decisionSubmit: any }): ReactElement => {
  ...
  const getTFR = (firstLevelPathKey: string): { value: string, label: string }[] => {
    switch (firstLevelPathKey) {
      case firstLevelPaths[0]:
        // 预警页人力链阈值波动范围选项
        // form.setFieldValue('attributtes',Object.keys(earlyWarningStaffTFR)[0])
      ...
      case firstLevelPaths[2]:
        // 预警页物流链阈值波动范围选项
        let as1:any=Object.keys(earlyWarningConveyTFR).map((item:any)=>{
          if(item==="运输费用1")
            return "运输费用"
          return item;
        })
        return as1.map((option:any) => ({
          value: option,
          label: option,
        }));
      ...
      case firstLevelPaths[3]:
        // 预警页生产链阈值波动范围选项
        // console.log("Object.keys(earlyWarningProductionTFR)",Object.keys(earlyWarningProductionTFR));
        let as:any=Object.keys(earlyWarningProductionTFR).map((item:any)=>{
          if(item==="生产费用1")
            return "生产费用"
          return item;
        })
  ...
  const onCompanyChange = (company: string) => {
    // 更新选择的公司并保earlyWarningOption
    console.log(earlyWarningOption);
  ...
  useEffect(() => {
    ...
    if (earlyWarningOption === undefined) {
      ...
      // console.log('earlyWarningOption',earlyWarningOption);
    ...
    >
      ...
      >
        ...
        <div>
          ...
          >
            <Select
            // defaultValue={getTFR(firstLevelPathKey)[0].value}
              showSearch
              optionFilterProp="children"
              // filterOption={filterOption}
              options={getTFR(firstLevelPathKey)}
              // onChange={value=>onTFRChange(value)}
            />
          </Form.Item>
        </div>
        <div>
