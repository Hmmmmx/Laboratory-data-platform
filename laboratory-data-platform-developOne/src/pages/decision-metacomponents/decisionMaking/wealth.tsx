...
const Wealth: FC = (): ReactElement => {
  ...
  const [topoOptions, setTopoOptions] = useState<EChartsOption>(topoOption);
  ...
  useEffect(() => {
    ...
    console.log(formData.current.corporation);
  }, [earlyWarningOption]);
  ...
  const getResDataWithCategories = (data: any) => {
    ...
    for (let i = 0; i < corporationList.length; i++) {
      for (let j = 0; j < dataWithCategories[corporationList[i]].length; j++) {
        dataWithCategories[corporationList[i]][j].categories =
  ...
  const getTopoGraph = (data: any, params: any) => {
    ...
    for (
      let j = 0;
      j < dataWithCategories[corporationList[index]].length;
      j++
    ) {
      //记录当前循环次数
      count++;
      ...
      graph.nodes.push({
        id: `${nodesCount + j}`,
        name: `${formatDate(
          dataWithCategories[corporationList[index]][j].eventTime,
        )}`,
        draggable: true,
        colors:
          dataWithCategories[corporationList[index]][j].level === 2
            ? red
            : dataWithCategories[corporationList[index]][j].level === 1
            ? green
            : yellow,
      ...
      graph.links.push({
        ...
        relation: {
          name: formatLevel(
            dataWithCategories[corporationList[index]][j].level,
          ),
        },
      });
    ...
    for (
      ...
      graph.nodes.push({
        ...
        name: `${'原因'}`,
        tooltipContent: `${
          dataWithCategories[corporationList[index]][j].causeType
        }`,
        draggable: true,
        colors:
          dataWithCategories[corporationList[index]][j].level === 2
    ...
    for (
      ...
      graph.nodes.push({
        id: `${nodesCount + j}`,
        // name: `${dataWithCategories[corporationList[index]][j].measure}`,
        name: `${'措施'}`,
        tooltipContent: `${
          dataWithCategories[corporationList[index]][j].measure
        }`,
