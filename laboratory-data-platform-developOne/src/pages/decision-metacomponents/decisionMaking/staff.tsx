  ...
  series: [
    {
      ...
      label: {
        ...
        align: 'center',
        formatter: function (params: any) {
          return params.data.displayName; // 使用 displayName 属性作为标签
        },
      },
      symbolSize: 90,
      roam: true,
...
const getTopoGraph = (data: any, params: any) => {
  ...
  //获取对应公司的索引
  const index = corporationList.findIndex((item) => item == params.corporation);
  // push公司  第二层节点
  ...
  for (let j = 0; j < data[corporationList[index]].length; j++) {
    ...
    graph.nodes.push({
      id: `${nodesCount + j}`,
      name: `${data[corporationList[index]][j].skill}`,
      draggable: true,
      colors:
  ...
  for (let j = 0; j < data[corporationList[index]].length; j++) {
    ...
    graph.nodes.push({
      // name: `${nodesCount + j}`, // 将 name 更改为数字
...
const Staff: FC = (): ReactElement => {
  ...
  const [, setCorporationOptions] = useState<any>(corporationOption);
  const [levelOptions, setLevelOptions] = useState<any>(levelOption);
  const [measureOptions, setMeasureOptions] = useState<any>(measureOption);
  const { firstLevelPathKey } = useCurrentPath(); // 当前路由
  const [topoOptions, setTopoOptions] = useState<any>(topoOption);
  ...
  const columns = [
    ...
    // },
    {
      ...
      render: (text: number, record: any) => {
        ...
        const level: string = `${formatDate(record.eventTime)}`;
        return level;
      },
    },
    {
